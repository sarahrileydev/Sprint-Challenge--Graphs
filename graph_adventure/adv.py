...
# Start looping through roomGraph
while len(graph) < len(roomGraph):

    # Get current room's unexplored exits and save to list to randomize
    unexplored_rooms = []
    for route in player.currentRoom.id:
        if player.currentRoom.id[route] == '?':
            unexplored_rooms.append(route)


    # If current room has unexplored_rooms picks a random unexplored direction from the player's current room
    if len(unexplored_rooms) > 0:
        random.shuffle(unexplored_rooms)
        # Pick a random unexplored_room
        route = unexplored_rooms[0]
        # Add this room to graph
        graph.append(route)
        # Move player
        player.travel(route)

    # Update rooms explored in graph
    
    else:
        # If no unexplored_rooms go back until we find one
        go_back = bfs(player.currentRoom.id)
